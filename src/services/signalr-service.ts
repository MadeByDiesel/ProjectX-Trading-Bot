...
export class SignalRService {
  ...
  private setupUserHubHandlers(): void {
    if (!this.userHubConnection) return;

    this.userHubConnection.on('GatewayUserAccount', (data: GatewayUserAccount) => {
      this.emit('account_update', data);
    });

    this.userHubConnection.on('GatewayUserOrder', (data: GatewayUserOrder) => {
      this.emit('order_update', data);
    });

    this.userHubConnection.on('GatewayUserPosition', (data: GatewayUserPosition) => {
      this.emit('position_update', data);
    });

    this.userHubConnection.on('GatewayUserTrade', (data: GatewayUserTrade) => {
      this.emit('trade_update', data);
    });

    this.userHubConnection.onreconnected(() => {
      this.logger.info('User Hub reconnected');
      this.subscribeToUserHub();
    });
  }

  private setupMarketHubHandlers(): void {
    if (!this.marketHubConnection) return;

    this.marketHubConnection.on('GatewayQuote', (contractId: string, data: GatewayQuote) => {
      this.emit('market_data', { contractId, ...data });
    });

    this.marketHubConnection.on('GatewayTrade', (contractId: string, data: GatewayTrade) => {
      this.emit('market_trade', { contractId, ...data });
    });

    this.marketHubConnection.on('GatewayDepth', (contractId: string, data: GatewayDepth) => {
      this.emit('market_depth', { contractId, ...data });
    });

    this.marketHubConnection.onreconnected(() => {
      this.logger.info('Market Hub reconnected');
    });
  }
  ...
  async subscribeToMarketData(contractId: string): Promise<void> {
    ...
    try {
      await this.marketHubConnection.invoke('SubscribeContractQuotes', contractId);
      await this.marketHubConnection.invoke('SubscribeContractTrades', contractId);
      this.logger.info(`Subscribed to market data for contract: ${contractId}`);
    } catch (error) {
  ...
  async unsubscribeFromMarketData(contractId: string): Promise<void> {
    ...
    try {
      await this.marketHubConnection.invoke('UnsubscribeContractQuotes', contractId);
      await this.marketHubConnection.invoke('UnsubscribeContractTrades', contractId);
  ...
  isConnected(): boolean {
    return (this.userHubConnection?.state === signalR.HubConnectionState.Connected) &&
